...
"""
            ...
            if self.queue.userVersion == 5 {
                try self.queue.execute(
"""
                    BEGIN TRANSACTION;
                    CREATE TABLE view_portrait_operation (
                        rowid INTEGER PRIMARY KEY NOT NULL,
                        device_identifier TEXT NOT NULL,
                    ...
                    CREATE TABLE view_fieldbook_operation (
                        rowid INTEGER PRIMARY KEY NOT NULL,
                        device_identifier TEXT NOT NULL,
                        timestamp TEXT NOT NULL,
                        timestampTz TEXT NOT NULL
...
"""
    ...
    func addViewFieldbook() throws {
        let viewFieldbookOperation = ViewFieldbookOperation(deviceIdentifier: Settings.deviceId(), timestamp: ZonedDateTime())
    ...
    func addViewPortrait(speciesId: Int64) throws {
        let viewPortraitOperation = ViewPortraitOperation(deviceIdentifier: Settings.deviceId(), speciesId: speciesId, timestamp: ZonedDateTime())
    ...
    func getPendingOperations() throws -> ([Int64], [Operation]) {
        let operationAndId = try queue.prepareRowIterator(
            Operation.D.table
                ...
                .join(
                    .leftOuter,
                    ViewPortraitOperation.D.table,
                    on: Operation.D.table[Operation.D.rowid] == ViewPortraitOperation.D.table[ViewPortraitOperation.D.rowid]
                )
                .join(
                    .leftOuter,
