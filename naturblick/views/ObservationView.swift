...
struct ObservationView: HostedView {
    ...
    private func speciesInfoNavigate() {
        if let species = model.observation?.species?.listItem {
            navigationController?.pushViewController(SpeciesInfoView(backend: backend, selectionFlow: false, species: species, flow: VoidSelectionFlow()).setUpViewController(), animated: true)
        }
    }
